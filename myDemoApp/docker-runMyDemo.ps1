...
if("stop" -ne $stop){
    ...
    $cmd = 'docker run --name=' + $myDemoName + ' -e MYDEMO_NAME=' + $demoName + ' -e MYDEMO_HOSTNAME=' + $myDemoHostName + ' -e MYDEMO_DB_PORT=' + $dbMappedPort + ' -e MYDEMO_DB_NAME=' + $dbName + ' --add-host ' + $myDemoHostName + ':' + '10.10.10.1' + ' -p ' +  $demoMappedPort + ':8080 -d -v $Env:MYDEMO_PATH\myDemo\target\myDemo-0.0.1-SNAPSHOT.jar:/docker/myDemo-0.0.1-SNAPSHOT.jar java:8u40 java -jar /docker/myDemo-0.0.1-SNAPSHOT.jar'
